(function(){
  ...
  function discard(){for(const [node,value] of originals)if(node.isConnected)node.textContent=value;setEditing(false)}
  async function persist(){
    const dirty=changed().filter(node=>node.isConnected);if(!dirty.length){setEditing(false);return}
