(function(){
  ...
  const quality=document.createElement('div'),activeQuality=state.quality||document.querySelector('.deck-slide')?.dataset.deckQuality||'good',qualityHelp={es:{good:'Look & feel de la presentación Admira',better:'Cada lámina con imagen temática del tema (Grok)',best:'Adaptación a la web o película elegida'},ca:{good:'Look & feel de la presentació d\'Admira',better:'Cada làmina amb imatge temàtica del tema (Grok)',best:'Adaptació al web o la pel·lícula escollida'},en:{good:'Admira presentation look and feel',better:'Each slide gets a thematic image for its topic (Grok)',best:'Adaptation to the selected website or film'}};quality.className='quality-levels';quality.setAttribute('aria-label','Good, Better and Best');quality.innerHTML=['good','better','best'].map(level=>`<button type="button" data-quality="${level}" aria-pressed="${level===activeQuality?'true':'false'}">${level[0].toUpperCase()+level.slice(1)}</button>`).join('');const localizeQuality=language=>{const labels=qualityHelp[language]||qualityHelp.es;quality.querySelectorAll('[data-quality]').forEach(button=>button.title=labels[button.dataset.quality])};localizeQuality(state.language);document.addEventListener('admira:language',event=>localizeQuality(event.detail?.language));quality.addEventListener('click',event=>{const button=event.target.closest('[data-quality]');if(!button||typeof window.__ADMIRA_APPLY_QUALITY__!=='function')return;window.__ADMIRA_APPLY_QUALITY__(button.dataset.quality);const url=new URL(location.href);url.searchParams.set('quality',button.dataset.quality);history.replaceState(null,'',url)});document.body.prepend(quality);
