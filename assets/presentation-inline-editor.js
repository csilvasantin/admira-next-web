(function(){
  ...
  style.textContent+='.inline-editor{top:68px}.quality-levels{position:fixed;z-index:31;left:18px;top:18px;display:flex;align-items:center;gap:5px;padding:4px;background:color-mix(in srgb,var(--surface) 84%,transparent);border:1px solid color-mix(in srgb,var(--ink) 18%,transparent);border-radius:calc(var(--radius) + 16px);backdrop-filter:blur(12px);box-shadow:0 8px 24px rgba(0,0,0,.14);font:800 9px/1 var(--mono);letter-spacing:.05em;text-transform:uppercase}.quality-levels span,.quality-levels strong{padding:8px 10px;border-radius:calc(var(--radius) + 10px);color:color-mix(in srgb,var(--ink) 60%,transparent)}.quality-levels strong{background:var(--accent);color:var(--bg)}@media(max-width:700px){.quality-levels{left:12px;top:12px}.inline-editor{top:auto}}';
  document.head.appendChild(style);
  const quality=document.createElement('div'),activeQuality=document.querySelector('.deck-slide')?.dataset.deckQuality||'best';quality.className='quality-levels';quality.setAttribute('aria-label','Good, Better and Best');quality.innerHTML=['good','better','best'].map(level=>`<${level===activeQuality?'strong':'span'}>${level[0].toUpperCase()+level.slice(1)}</${level===activeQuality?'strong':'span'}>`).join('');document.body.prepend(quality);
