(function(){
  ...
  const quality=document.createElement('div'),activeQuality=state.quality||document.querySelector('.deck-slide')?.dataset.deckQuality||'good',qualityHelp={good:'Look & feel de la presentación Admira',better:'Dirección editorial definida por Codex',best:'Adaptación a la web o película elegida'};quality.className='quality-levels';quality.setAttribute('aria-label','Good, Better and Best');quality.innerHTML=['good','better','best'].map(level=>`<button type="button" data-quality="${level}" aria-pressed="${level===activeQuality?'true':'false'}" title="${qualityHelp[level]}">${level[0].toUpperCase()+level.slice(1)}</button>`).join('');quality.addEventListener('click',event=>{const button=event.target.closest('[data-quality]');if(!button||typeof window.__ADMIRA_APPLY_QUALITY__!=='function')return;window.__ADMIRA_APPLY_QUALITY__(button.dataset.quality);const url=new URL(location.href);url.searchParams.set('quality',button.dataset.quality);history.replaceState(null,'',url)});document.body.prepend(quality);
