(function(){
  'use strict';
  window.__ADMIRA_GENERATOR_VERSION__='20260718-7';
  ...
  const presentationTypes=[
    {key:'classic',tier:'Good',label:'Classic',duration:1,description:'Esencial · audio y vídeo de 1 minuto.',primary:'#172b55',accent:'#f5a623'},
    {key:'admira',tier:'Better',label:'Admira',duration:3,description:'Ejecutiva y cuadrática · audio y vídeo de 3 minutos.',primary:'#071a2f',accent:'#3df08a'},
    {key:'movie',tier:'Best',label:'Película',duration:5,description:'Inmersiva y narrativa · audio y vídeo de 5 minutos.'}
  ];
  ...
  function applyPresentationStyle(style=currentPresentationStyle(),applyColors=true){
    ...
    document.querySelectorAll('[data-duration-output]').forEach(node=>{node.textContent=`${type.duration} min`});
    const levelHelp=document.getElementById('outputLevelHelp'); if(levelHelp)levelHelp.textContent=`${type.tier} / ${type.label}: audio y vídeo de ${type.duration} minuto${type.duration===1?'':'s'} por idioma; el mismo acabado se aplica a todos los documentos.`;
  ...
  outputPanel.innerHTML='<h2>5. ¿Qué queremos obtener?</h2><p class="sub" id="outputLevelHelp">Best / Película: audio y vídeo de 5 minutos por idioma; el mismo acabado se aplica a todos los documentos.</p><div class="output-grid"><label class="output"><input type="checkbox" name="output" value="website" checked><b>01</b><span>Website</span></label><label class="output"><input type="checkbox" name="output" value="audio" checked><b>02</b><span>Audio · <i data-duration-output>5 min</i></span></label><label class="output"><input type="checkbox" name="output" value="video" checked><b>03</b><span>Vídeo · <i data-duration-output>5 min</i></span></label><label class="output"><input type="checkbox" name="output" value="pdf" checked><b>04</b><span>PDF</span></label><label class="output"><input type="checkbox" name="output" value="powerpoint" checked><b>05</b><span>PowerPoint</span></label><label class="output"><input type="checkbox" name="output" value="documents" checked><b>06</b><span>Documento de trabajo</span></label><label class="output"><input type="checkbox" name="output" value="infographic" checked><b>07</b><span>Infografía</span></label><label class="output all"><input type="checkbox" id="allOutputs" checked><b>08</b><span>Todo</span></label></div>';
  ...
  function renderGeneration(generation){
    ...
    const languageNames={es:'Castellano',ca:'Català',en:'English'},states={waiting:'Falta traducción',queued:'En cola',processing:'Produciendo',ready:'Listo',published:'Publicado',failed:'Error'};
    createdMatrix.innerHTML=(generation.languages||[]).map(language=>{const items=tasks.filter(task=>task.language===language).map(task=>{const timing=task.actualDurationSeconds?` · real ${Math.floor(task.actualDurationSeconds/60)}:${String(Math.round(task.actualDurationSeconds%60)).padStart(2,'0')}`:task.targetDurationSeconds?` · ${task.targetDurationSeconds/60} min`:'';return `<div class="created-task ${task.status}"><span>${task.label}${timing}</span><small>${states[task.status]||task.status}</small></div>`}).join('');return `<section class="created-language"><h3>${language.toUpperCase()} · ${languageNames[language]||language} · ${generation.productionLevel?.label||''} / ${generation.productionLevel?.look||''}</h3><div class="created-tasks">${items}</div></section>`}).join('');
  }
