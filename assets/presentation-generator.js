(function(){
  'use strict';
  window.__ADMIRA_GENERATOR_VERSION__='20260719-7';
  ...
  form.addEventListener('submit',async event=>{
    ...
    try{
      ...
      document.getElementById('openIdeas').href=body.ideasUrl; const openDeck=document.getElementById('openDeck');openDeck.href=body.deckUrl;openDeck.hidden=!body.outputs.includes('website'); currentClient=body.slug;currentGenerationUrl=`/presentaciones/${body.slug}/api/generation`; renderGeneration(body.generation);renderImageSet(null,body.slideCount||0);result.classList.add('show'); result.scrollIntoView({behavior:'smooth',block:'center'}); message(`Presentación lista: ${body.displayName}. Ya puedes abrirla y trabajar con ella.`);if(body.outputs.includes('backgrounds'))runImageGeneration(false).catch(error=>imageMessage(`La presentación sigue lista sin los fondos pendientes. Grok no ha respondido: ${error.message} Puedes reintentarlo cuando quieras.`,true));else imageApi({action:'prepare',client:body.slug}).then(response=>renderImageSet(response.imageSet)).catch(()=>loadImages(body.slug).catch(()=>{}));
