...
test('admin crea usuario, cambia rol y revoca inmediatamente la cookie anterior',async()=>{
  ...
  assert.deepEqual(audit.results.map(x=>x.action).sort(),['invite_email','user_created','user_updated','whitelist_add_failed']);
});
...
test('Webmaster sólo entrega y permite modificar los proyectos autorizados',async()=>{
  ...
  try{
    ...
    assert.deepEqual(body.proyectos.map((p)=>p.clave),['admiranext','admiranext-webmaster','admiranext-proyectos','generador-presupuestos','generador-de-presentaciones','presentaciones']);
