...
test('a client presentation renders the curated decks around the dynamic proposal',async()=>{
  ...
  assert.match(html,/data-section="proposal"/);assert.match(html,/data-label-ca="Què fem"/);assert.match(html,/data-label-en="What we propose"/);assert.match(html,/data-deck-progress-label data-title-es="Quiénes somos" data-title-ca="Qui som"/);assert.match(html,/data-alt-ca="Admira"/);assert.match(html,/data-src-better-es="\/presentaciones\/demo\/deck\/admira-2026\/best-team\.webp"/);assert.match(html,/data-src-best-es="\/presentaciones\/demo\/images\/site-inspired\.jpg"/);assert.match(html,/data-title-ca="Qui som"/);assert.match(html,/data-src-good-en=/);
});
