...
test('generator and presentation route expose the four narrative segments',async()=>{
  ...
  assert.match(gallery,/class="card control" href="\/presentaciones\/control\/"/);assert.match(gallery,/class="card generator" href="\/presentaciones\/"/);assert.match(gallery,/Generador · elige del censo/);assert.match(gallery,/Abrir generador/);
});
