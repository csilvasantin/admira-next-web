...
test('generator and presentation route expose the four narrative segments',async()=>{
  ...
  assert.match(gallery,/class="card control" href="\/presentaciones\/control\/"/);assert.match(gallery,/class="card generator" href="\/presentaciones\/"/);assert.match(gallery,/Presentador de presentaciones/);assert.match(gallery,/Abrir presentador/);
});
...
test('a client presentation renders the curated decks around the dynamic proposal',async()=>{
  ...
  assert.equal((html.match(/data-section="who"/g)||[]).length,10);assert.equal((html.match(/data-section="what"/g)||[]).length,10);assert.equal((html.match(/data-section="how"/g)||[]).length,22);assert.equal((html.match(/data-section-target=/g)||[]).length,5);assert.match(html,/data-section="budget"/);assert.match(html,/data-label-es="Presupuesto"/);
