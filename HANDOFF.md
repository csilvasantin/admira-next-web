...
| Consejo de Silicio (+ mesa de control) | https://www.admira.live | `csilvasantin/32.-ConsejoAdmiraNextGame` | homepage `v.26.06.18.r1` · mesa/control `v.26.06.18.r3` | `ac5c0c8` |
...
- **Monitor por máquina** (`GET /api/council/machine-status`): online, cuenta Claude logueada, versión de Claude Code, si está corriendo. Alcanzabilidad por SSH real (hostname Tailscale → IP), no por ping. Badge por máquina en la mesa.
...
- **Acciones de control acotadas** (`POST /api/teamwork/machine-action`, lista blanca): `claude-open`, `claude-quit` (abrir/cerrar Claude Code por SSH+osascript). Sin comando libre ni sudo. Botones ▶ Abrir / ■ Cerrar por máquina.
...
- admira.live homepage (badge r1) y mesa (r3) tienen versiones distintas: normal (páginas con cadencia propia), pero unificar el número visible si molesta.
...
1. Más acciones de control acotadas si hacen falta (p. ej. reiniciar app, refrescar captura) — siempre lista blanca, nunca comando libre/sudo.
2. Llevar el monitor de cuentas también a las máquinas worker (PC) si procede.
3. Revisar por qué `admira.live/` muestra r2 cuando el badge del homepage es r1 (posible cadena de versiones en index.html).
