...
export async function onRequest(context){
  ...
  if (isFormPost && !isGeneratorApi) {
    ...
    if (form.get('intent') === 'google') {
      const googleIdentity = ownerAllowed ? await verifyGoogleCredential(String(form.get('credential') || '')) : null;
      if (!googleIdentity) {
      ...
      headers.append('Set-Cookie', `pres_owner=${ownerToken}; Path=/presentaciones; Max-Age=${MAXAGE}; HttpOnly; Secure; SameSite=Lax`);
      headers.append('Set-Cookie', identityCookie(identityToken, MAXAGE));
      context.waitUntil(writeAccessEvent(env, request, {type:'google_owner_login', client:'generador', presentation:title, identity:googleIdentity, access:'owner', path:url.pathname}));
      return new Response(null, {status:303, headers});
    }
