...
// Estética del folleto (FLT-100372, Carlos 12-sep-2026): «por defecto hay que
// coger la estética del catálogo de referencia para que se vea una continuidad
// de marca; todos los vídeos son muy oscuros cuando el catálogo es blanco». El
// catálogo manda en el deep-link `estilo` {fondo, claro, colores[], tinta}
// medido sobre la portada; aquí se sanea y se convierte en una instrucción
// literal para el director creativo. Sin estilo, el folleto de supermercado se
// asume CLARO: es lo que son casi todos.
const HEX_RE = /^#[0-9a-f]{6}$/i;
const FORMATOS = new Set(['9:16', '16:9']);
export function normalizeEstilo(raw){
  if(!raw || typeof raw !== 'object') return null;
  const fondo = HEX_RE.test(String(raw.fondo || '')) ? String(raw.fondo).toLowerCase() : '';
  const tinta = HEX_RE.test(String(raw.tinta || '')) ? String(raw.tinta).toLowerCase() : '';
  const colores = (Array.isArray(raw.colores) ? raw.colores : []).map(c => String(c || '').toLowerCase()).filter(c => HEX_RE.test(c)).slice(0, 4);
  const claro = raw.claro == null ? true : Boolean(raw.claro);
  if(!fondo && !colores.length) return null;
  return {fondo, tinta, colores, claro, origen:clean(raw.origen, 40)};
}
export function normalizeFormatos(raw){
  const lista = (Array.isArray(raw) ? raw : [raw]).map(v => String(v || '').trim()).filter(v => FORMATOS.has(v));
  return lista.length ? [...new Set(lista)] : ['9:16'];
}
function nombreColor(hex){
  const r = parseInt(hex.slice(1, 3), 16), g = parseInt(hex.slice(3, 5), 16), b = parseInt(hex.slice(5, 7), 16);
  const max = Math.max(r, g, b), min = Math.min(r, g, b);
  if(max - min < 28) return max > 200 ? 'blanco' : max < 70 ? 'negro' : 'gris';
  const h = max === r ? ((g - b) / (max - min)) % 6 : max === g ? (b - r) / (max - min) + 2 : (r - g) / (max - min) + 4;
  const deg = (h * 60 + 360) % 360;
  if(deg < 15 || deg >= 345) return 'rojo';
  if(deg < 40) return 'naranja';
  if(deg < 70) return 'amarillo';
  if(deg < 165) return 'verde';
  if(deg < 200) return 'turquesa';
  if(deg < 260) return 'azul';
  if(deg < 300) return 'violeta';
  return 'magenta';
}
export function fraseEstilo(estilo, formatos){
  const f = normalizeFormatos(formatos);
  const formato = f.length === 2 ? 'dos versiones, vertical 9:16 y horizontal 16:9, con la misma idea' : f[0] === '16:9' ? 'vídeo horizontal 16:9' : 'vídeo vertical 9:16';
  const e = estilo || {fondo:'#ffffff', claro:true, colores:[], tinta:''};
  const colores = e.colores.length ? e.colores.map(c => `${nombreColor(c)} (${c})`).join(', ') : 'los de la marca del folleto';
  return e.claro
    ? `ESTÉTICA OBLIGATORIA (continuidad de marca con el folleto): escena luminosa y limpia, fondo claro${e.fondo ? ` (${nombreColor(e.fondo)}, ${e.fondo})` : ''}, luz de día, colores de marca ${colores}, producto real como protagonista, estilo de folleto de supermercado moderno. Nada de escenas oscuras, nocturnas, neón ni ciencia ficción. Formato: ${formato}.`
    : `ESTÉTICA OBLIGATORIA (continuidad de marca con el folleto): fondo oscuro${e.fondo ? ` (${e.fondo})` : ''} con los colores de marca ${colores}, producto real como protagonista, contraste alto y limpio. Formato: ${formato}.`;
}
...
export function normalizeProducto(raw){
  ...
  const producto = {
    ...
    validez,
    estilo:normalizeEstilo(raw.estilo),
    formatos:normalizeFormatos(raw.formatos)
  };
  producto.estiloTexto = fraseEstilo(producto.estilo, producto.formatos);
  ...
  const response = await fetch('https://api.x.ai/v1/responses', {
    ...
    body:JSON.stringify({
      ...
      input:[
        ...
        {
          role:'user',
          content:[{type:'input_text', text:JSON.stringify({mode:mode === 'producto' ? 'catalog_product' : mode === 'brief' ? 'campaign_brief' : mode === 'create' ? 'create_from_scratch' : 'develop_headline', headline:headline || null, producto:producto ? {nombre:producto.nombre, marca:producto.marca || null, detalle:producto.detalle || null, precio:producto.precioTexto || null, promo:producto.promo || null, validez:producto.validezTexto || null, catalogo:producto.catalogo || null, estetica:producto.estiloTexto, formatos:producto.formatos} : null, brief:brief ? {campana:brief.campana, tipologia:brief.tipologia, lane:brief.lane || null, publico:brief.publico, titulo:brief.titulo, mensaje:brief.mensaje || null, claim:brief.claim, marca:brief.marca} : null, format:brief ? `vídeo vertical de ${brief.duracion} segundos (${brief.formato})` : producto ? `vídeo de 15 segundos (${producto.formatos.join(' y ')})` : 'vídeo vertical de 15 segundos', language:'es'})}]
        }
      ],
