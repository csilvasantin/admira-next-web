 ...
 */
export function etiquetasCatalogo(catalogo) {
  if (!catalogo) return [...ETIQUETAS_BASE];
  return [...ETIQUETAS_BASE, 'catalogo', catalogo.cliente, catalogo.id, mesDe(catalogo.desde)].filter(Boolean);
}
...
export function saneaFicha(bruto) {
  ...
  const tags = catalogo
    ? [...new Set([...etiquetasCatalogo(catalogo), ...propias])].slice(0, MAX_ETIQUETAS_CATALOGO)
    : [...new Set([...ETIQUETAS_BASE, ...propias])].slice(0, MAX_ETIQUETAS);
