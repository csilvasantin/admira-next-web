...
export async function onRequestGet(context){
  ...
  function syncBestFigures(){document.querySelectorAll('.slide[data-image-index]').forEach(slide=>{const figure=slide.querySelector('.best-figure img');if(!figure)return;const raw=(slide.style.getPropertyValue('--slide-image')||'').trim();const match=raw.match(/^url\((['"]?)(.+)\1\)$/);const url=match?match[2]:'';if(url&&presentationState.quality==='best'&&slide.dataset.hasImage==='true'){if(figure.getAttribute('src')!==url)figure.setAttribute('src',url);figure.hidden=false}else{figure.removeAttribute('src');figure.hidden=true}})}
