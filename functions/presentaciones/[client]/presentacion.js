...
export async function onRequestGet(context){
  ...
  </style><style>html[data-quality="good"]{--primary:#e9ece2;--accent:#789b3a;--bg:#f7f8f3;--surface:#fff;--ink:#18212a;--sans:Inter,-apple-system,"Segoe UI",Arial,sans-serif}html[data-quality="better"]{--primary:#12233e;--accent:#ffb000;--bg:#07101b;--surface:#111827;--ink:#f8fbff;--sans:Inter,-apple-system,"Segoe UI",Arial,sans-serif}html[data-quality="good"] .slide:not(.deck-slide){background:linear-gradient(135deg,#fff,#edf0e7)}html[data-quality="good"] .slide:not(.deck-slide):after{border-color:rgba(120,155,58,.13)}html[data-quality="better"] .slide:not(.deck-slide){background:radial-gradient(90% 80% at 85% 0%,rgba(255,176,0,.24),transparent 62%),linear-gradient(145deg,#12233e,#07101b)}html[data-quality="good"] .slide[data-has-image="true"]:before,html[data-quality="better"] .slide[data-has-image="true"]:before{display:none}.deck-slide,.slide{transition:background .35s,color .35s}.deck-best .deck-copy h2{font-family:var(--sans);font-weight:750}.deck-best .deck-kicker{color:var(--accent)}html[data-quality="best"] .deck-best img{filter:saturate(.94) contrast(1.08) brightness(.66)}.section-nav{position:fixed;z-index:20;left:50%;top:18px;transform:translateX(-50%);display:flex;gap:3px;padding:4px;border:1px solid color-mix(in srgb,var(--ink) 15%,transparent);border-radius:999px;background:color-mix(in srgb,var(--surface) 78%,transparent);backdrop-filter:blur(12px)}.section-nav button{display:flex;align-items:center;gap:7px;border:0;border-radius:999px;padding:8px 11px;background:transparent;color:color-mix(in srgb,var(--ink) 64%,transparent);font:750 10px/1 var(--sans);cursor:pointer;white-space:nowrap}.section-nav button b{font:800 9px/1 var(--mono);color:var(--accent)}.section-nav button[aria-pressed="true"]{background:var(--accent);color:var(--bg)}.section-nav button[aria-pressed="true"] b{color:inherit}.section-nav button:disabled{display:none}@media(max-width:1050px){.section-nav button{padding:9px}.section-nav button span{display:none}}@media(max-width:650px){.section-nav{top:auto;bottom:14px}.section-nav button{padding:8px 10px}}</style></head><body><div class="languages" aria-label="Idiomas">${buttons}</div><nav class="section-nav" aria-label="Segmentos de la presentación">${sectionNav}</nav>${beforeSlides}<section class="slide cover"${mediaFor('cover')?' data-slide-media':''} data-slide-key="cover" data-segment="proposal" data-section="proposal" data-image-index="0"${imageAttr(0)}>${mediaFor('cover')}<div class="inner"><span class="eyebrow" data-edit-field="hero.eyebrow">${esc(ideas.hero?.eyebrow||`ADmiraNeXT × ${config.displayName}`)}</span><h1 id="coverTitle" data-edit-field="hero.title">${esc(ideas.hero?.title)}</h1><p class="detail" id="coverSummary" data-edit-field="hero.summary">${esc(ideas.hero?.summary)}</p></div></section><section class="slide"${mediaFor('objective')?' data-slide-media':''} data-slide-key="objective" data-segment="proposal" data-section="proposal" data-image-index="1"${imageAttr(1)}>${mediaFor('objective')}<div class="inner"><span class="eyebrow" id="objectiveLabel" data-edit-field="labels.objective">${esc(locales.es.labels.objective)}</span><h2 id="objectiveText" data-edit-field="objective">${esc(ideas.objective)}</h2></div></section>${slides}<section class="slide"${mediaFor('closing')?' data-slide-media':''} data-slide-key="closing" data-segment="proposal" data-section="proposal" data-image-index="${baseBlocks.length+2}"${imageAttr(baseBlocks.length+2)}>${mediaFor('closing')}<div class="inner close"><span class="eyebrow" id="nextLabel" data-edit-field="labels.next">${esc(locales.es.labels.next)}</span><strong id="closingTitle" data-edit-field="closing.title">${esc(ideas.closing?.title)}</strong><span id="closingAction" data-edit-field="closing.action">${esc(ideas.closing?.action)}</span></div></section>${afterSlides}<div class="nav" id="sequenceNav">↑ ↓ · F pantalla completa</div><script>
