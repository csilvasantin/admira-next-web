...
function visibleBlock(value={}){
  ...
  return {
    id:value.id||'',title:value.title||'',message:value.message||'',detail:value.detail||'',enabled:value.enabled!==false,
    act:value.act||'',role:value.role||'',chapter:value.chapter||'',product:value.product||'',promise:value.promise||'',
    ...tiempo
  };
}
...
const uiLabels={
  es:{print:'Descargar en PDF (imprimir)',languages:'Idiomas',sections:'Segmentos de la presentación',presentation:'Presentación',fullscreen:'Pantalla completa',fullscreenActive:'Pantalla completa activa · Esc para salir',logoAlt:'Logo de cliente'},
  ca:{print:'Descarregar en PDF (imprimir)',languages:'Idiomes',sections:'Segments de la presentació',presentation:'Presentació',fullscreen:'Pantalla completa',fullscreenActive:'Pantalla completa activa · Esc per sortir',logoAlt:'Logotip del client'},
  en:{print:'Download as PDF (print)',languages:'Languages',sections:'Presentation sections',presentation:'Presentation',fullscreen:'Full screen',fullscreenActive:'Full screen active · Esc to exit',logoAlt:'Client logo'}
};
...
/** Láminas de otra presentación (slug) como segmento before/after/insert. */
function presentationRefSlides(refConfig,refIdeas,position,{hostClient=''}={}){
  if(!refConfig||!refIdeas)return '';
  const blocks=(refIdeas.skeleton||[]).filter(item=>item.enabled!==false).map(visibleBlock);
  const label=esc(refConfig.displayName||refConfig.slug||'Sala');
  const sourceSlug=esc(refConfig.slug||'');
  return blocks.map((item,index)=>{
    const key=`${sourceSlug}-${item.id||index}`;
    return `<section class="slide insert-slide" data-segment="${esc(position)}" data-section="proposal" data-insert-slug="${sourceSlug}" data-slide-key="${esc(key)}" data-block-id="${esc(item.id||`idea-${index+1}`)}"><div class="inner"><span class="eyebrow">${label}</span><span class="num">${String(index+1).padStart(2,'0')}</span><h2>${esc(item.title)}</h2><p class="message">${esc(item.message)}</p><p class="detail">${esc(item.detail)}</p></div></section>`;
  }).join('');
}

function footerMarkup(footer,slideLabel=''){
  if(!footer)return '';
  const parts=[];
  if(footer.showBrand!==false)parts.push(`<span class="slide-footer-brand">${esc(footer.text||'ADmiraNeXT')}</span>`);
  else if(footer.text)parts.push(`<span class="slide-footer-text">${esc(footer.text)}</span>`);
  if(footer.showSlideNumber!==false&&slideLabel)parts.push(`<span class="slide-footer-num">${esc(slideLabel)}</span>`);
  if(!parts.length)return '';
  return `<footer class="slide-footer" data-slide-footer>${parts.join('')}</footer>`;
}

async function loadPresentationRef(env,slug){
  if(!slug||!env?.PRESENTATION_IDEAS)return null;
  const [config,ideas]=await Promise.all([
    env.PRESENTATION_IDEAS.get(`presentation:${slug}`,{type:'json'}),
    env.PRESENTATION_IDEAS.get(`ideas:${slug}`,{type:'json'})
  ]);
  if(!config||!ideas)return null;
  return {config,ideas};
}

function resolveSegmentHtml(sequenceField,kind,client,position,options,refCache){
  if(!sequenceField)return '';
  if(kind==='presentation'){
    const ref=refCache.get(sequenceField);
    return ref?presentationRefSlides(ref.config,ref.ideas,position,{hostClient:client}):'';
  }
  return deckSlides(sequenceField,client,position,options);
}
...
export async function onRequestGet(context){
  ...
  const bestVideoAttr=slide=>{const entry=slideMedia.find(e=>e.slide===String(slide).toLowerCase());const source=entry&&entry.type==='video'?String(entry.effectiveSrc||''):'';return source?` data-best-video="${esc(source)}"`:''};
  ...
  const foot=(label)=>footerMarkup(footer,label);
  const initialUi=uiLabels[idiomaInicial]||uiLabels.es;
  const buttons=(logo?`<button type="button" class="brand-identity" data-client-fullscreen aria-pressed="false" aria-label="${esc(initialUi.fullscreen)}" title="${esc(initialUi.fullscreen)}"><span class="brand-mark"><img src="${logo}" alt="${esc(initialUi.logoAlt)} · ${name}"></span><span class="brand-name">${name}</span></button>`:'')+languages.map((language,index)=>`<button type="button" data-language="${esc(language)}" aria-pressed="${index===0?'true':'false'}">${esc(language.toUpperCase())}</button>`).join('')+`<button type="button" class="deck-print" data-print aria-label="${esc(initialUi.print)}" title="${esc(initialUi.print)} · P">PDF</button>`;
  const initialQuality=option(config.sequence?.beforeQuality,['good','better','best'],'good');
  // Un set cerrado (ninguna lamina en estado no terminal) no tiene nada que generar:
  // que la sala llame a la API del generador solo sirve para que a los invitados les
  // conteste 401 con la pagina de login y para gastar dos viajes en cada visita.
  const setCerrado=Array.isArray(imageSet?.slides)&&imageSet.slides.length>0&&imageSet.slides.every(lamina=>lamina?.status==='ready'||lamina?.status==='failed');
  // Y aunque quede lámina por llegar, sólo genera quien tiene permiso para la API del
  // generador (MorfeoMacMini, 21-09-2026 · FLT-100766 b): el middleware lo deja en
  // context.data. El invitado sigue sondeando api/images —lectura de SU sala— y ve
  // aparecer las láminas que genere otro, pero no dispara un POST que le da 401.
  const puedeGenerar=context.data?.presentationAccess?.canGenerate===true;
  const generaLaminas=!setCerrado&&puedeGenerar;
  // FLT-100646 (Lucas, CSO): la sala WEB abria en GOOD, y en GOOD el fondo de las
  // laminas no se pinta. Con 9 de 11 laminas ilustradas, el cliente recibia 120 KB
  // de texto sobre un degradado —«un PDF en el navegador»— y las imagenes
  // generadas no se veian salvo que alguien supiera pulsar BEST. Si la portada
  // tiene arte y lo tiene al menos la mitad del deck, la sala abre en BEST; si no,
  // manda lo que diga el generador. `initialQuality` sigue rigiendo los decks
  // corporativos (before/after), que es otra decision.
  const laminasSala=baseBlocks.length+3;
  const laminasConArte=Array.from({length:laminasSala},(_,index)=>imageAttr(index)).filter(Boolean).length;
  const salaConArte=Boolean(imageAttr(0))&&laminasConArte>=Math.ceil(laminasSala/2);
  const calidadSala=salaConArte?'best':initialQuality;
  const refSlugs=new Set();
  if(config.sequence?.beforeKind==='presentation'&&config.sequence?.before)refSlugs.add(config.sequence.before);
  if(config.sequence?.afterKind==='presentation'&&config.sequence?.after)refSlugs.add(config.sequence.after);
  for(const insert of config.sequence?.inserts||[]) if(insert?.slug)refSlugs.add(insert.slug);
  const refCache=new Map();
  await Promise.all([...refSlugs].map(async slugRef=>{
    const loaded=await loadPresentationRef(context.env,slugRef);
    if(loaded)refCache.set(slugRef,loaded);
  }));
  const deckOptions={length:config.sequence?.beforeLength,quality:initialQuality,adaptedImages};
  const beforeSlides=resolveSegmentHtml(config.sequence?.before,config.sequence?.beforeKind,client,'before',deckOptions,refCache);
  const afterSlides=resolveSegmentHtml(config.sequence?.after,config.sequence?.afterKind,client,'after',{quality:initialQuality,adaptedImages},refCache);
  const insertsByAfter=new Map();
  for(const insert of config.sequence?.inserts||[]){
    const key=String(insert.afterBlock||'').toLowerCase();
    const list=insertsByAfter.get(key)||[];
    const ref=refCache.get(insert.slug);
    if(ref)list.push(presentationRefSlides(ref.config,ref.ideas,'insert',{hostClient:client}));
    insertsByAfter.set(key,list);
  }
  const insertAfter=(blockId)=>(insertsByAfter.get(String(blockId||'').toLowerCase())||[]).join('');
  const earlyInserts=insertAfter('');
  const slides=baseBlocks.map((bloqueBase,index)=>{
    const item=enIdiomaInicial(bloqueBase,index);
    const key=String(item.id||`idea-${index+1}`).toLowerCase();
    const actAttr=item.act?` data-act="${esc(item.act)}"`:'';
    ...
    return `<section class="slide"${presenterSeconds(item)}${mediaFor(key)?' data-slide-media':''} data-slide-key="${esc(key)}" data-segment="proposal" data-section="proposal" data-image-index="${index+2}" data-block="${index}" data-block-id="${esc(item.id||`idea-${index+1}`)}"${actAttr}${roleAttr}${chapterAttr}${imageAttr(index+2)}${bestVideoAttr(key)}>${mediaFor(key)}<figure class="best-figure" aria-hidden="true"><img alt="" loading="lazy"><video class="best-figure-video" playsinline muted loop preload="metadata" hidden></video></figure><div class="inner"><span class="num">${String(index+1).padStart(2,'0')}</span>${productEyebrow}<h2 data-edit-field="skeleton.title">${esc(item.title)}</h2><p class="message" data-edit-field="skeleton.message">${esc(item.message)}</p><p class="detail" data-edit-field="skeleton.detail">${esc(item.detail)}</p></div>${foot(`${index+1}/${baseBlocks.length}`)}</section>${insertAfter(item.id)}`;
  }).join('');
  const coverInsert=insertAfter('cover');
  const objectiveInsert=insertAfter('objective');
  ...
  // HERO «GAME BOY» (Carlos, 19-09-2026 · FLT-100646). Salida web con la referencia visual
  // emulada (look & feel de una web tipo voicebenchmarks): un dispositivo retro dot-matrix que
  // abre el deck. Opt-in por theme.heroDevice='pocket'; se puebla con los datos del propio deck.
  const heroDevice=option(config.theme?.heroDevice,['none','pocket'],'none');
  const pocketFont=heroDevice==='pocket'?'<link href="https://fonts.googleapis.com/css2?family=VT323&display=swap" rel="stylesheet">':'';
  const pocketWord=esc((String(config.displayName||'DECK').trim().split(/\s+/)[0]||'DECK').toUpperCase());
  const pocketHero=heroDevice!=='pocket'?'':`<section class="slide pocket-hero" data-slide-key="pocket" data-segment="proposal" data-section="proposal"><div class="gbx">
    <div class="gbx-power"><span class="slot"><span class="sld"></span></span>◄ OFF · ON ►</div>
    <div class="gbx-bezel"><div class="gbx-dm">DOT MATRIX WITH STEREO SOUND</div><div class="gbx-batt"><span class="led"></span>BATTERY</div>
      <div class="gbx-lcd"><div class="r head"><span>▤ ${esc(String(config.displayName||'').toUpperCase())} / DECK</span><span>▦▦▦</span></div>
        <div class="col">${esc(('La presentación de '+(config.displayName||'')).toUpperCase())}</div>
        <div class="mm">Menú principal <span class="cur">_</span></div>
        <p class="intro">Explora la propuesta.<br>Preséntala en vivo o en PDF.</p>
        <div class="it sel"><span>▶ Ver presentación</span><span>›</span></div>
        <div class="it"><span>&nbsp;&nbsp;Descargar PDF</span><span>›</span></div>
        <div class="cnt">${baseBlocks.length} LÁMINAS / ${languages.length} IDIOMA${languages.length===1?'':'S'}</div>
        <div class="r foot"><span>↑↓ SELECT · A: ABRIR</span><span>01 / ${String(baseBlocks.length+2).padStart(2,'0')}</span></div></div></div>
    <div class="gbx-word"><b>${pocketWord}</b><i>pocket</i><sup>01</sup></div>
    <div class="gbx-ctl"><div class="gbx-dpad"><span class="h"></span><span class="v"></span><span class="c"></span></div>
      <div class="gbx-ab"><div class="k"><div class="btn"></div><span class="lbl">B</span></div><div class="k"><div class="btn"></div><span class="lbl">A</span></div></div></div>
    <div class="gbx-se"><div class="k"><div class="cap"></div><span class="lbl">SELECT</span></div><div class="k"><div class="cap"></div><span class="lbl">START</span></div></div>
    <div class="gbx-phones">PHONES <span class="jack"></span></div>
    <div class="gbx-grille"><i></i><i></i><i></i><i></i><i></i><i></i></div>
  </div></section>`;
  const pocketCss=heroDevice!=='pocket'?'':`html[data-mode]{--bg:${background}!important;--surface:${surface}!important;--ink:${text}!important;--primary:${primary}!important;--accent:${accent}!important;--sans:${fontStack(config.theme?.fontStyle)}!important}html[data-mode] .slide:not(.deck-slide){background:radial-gradient(75% 60% at 84% -6%,color-mix(in srgb,var(--accent) 12%,transparent),transparent 62%),var(--bg)!important;color:var(--ink)}html[data-mode] .slide:not(.deck-slide):not([data-has-image]):not([data-has-video]):before{display:none!important}html[data-mode] .slide:not(.deck-slide):after{border-color:color-mix(in srgb,var(--accent) 12%,transparent)!important}.pocket-hero{background:var(--bg)!important;place-items:center}.pocket-hero:after{display:none!important}.pocket-hero .gbx{position:relative;width:min(420px,90vw);padding:28px 20px;background:linear-gradient(158deg,#f1eee5,#e4e1d7 58%,#d6d3c9);border-radius:16px 16px 18px 64px;border:1px solid #cecbc1;box-shadow:0 34px 74px rgba(0,0,0,.24),inset 0 2px 0 rgba(255,255,255,.9),inset 0 -4px 10px rgba(0,0,0,.10);font-family:Inter,-apple-system,"Segoe UI",Arial,sans-serif;color:#303d31}.gbx-power{position:absolute;top:-13px;left:30px;display:flex;align-items:center;gap:13px;font-size:12px;font-weight:700;letter-spacing:.1em;color:#5f6a42}.gbx-power .slot{display:flex;align-items:center;justify-content:center;width:60px;height:24px;padding:4px;border-radius:10px 10px 7px 7px;background:linear-gradient(#ece9e0,#d7d4cb);border:1px solid #c2bfb5;box-shadow:inset 0 2px 4px rgba(0,0,0,.22),0 1px 0 rgba(255,255,255,.9)}.gbx-power .sld{width:36px;height:14px;border-radius:4px;border:1px solid #95929a;background:repeating-linear-gradient(90deg,#a7a5ac 0 2px,#d2d0d7 2px 4px);box-shadow:0 1px 2px rgba(0,0,0,.35),inset 0 1px 0 rgba(255,255,255,.6)}.gbx-bezel{position:relative;background:linear-gradient(160deg,#938f9b,#7d7986);border-radius:9px 9px 44px 9px;padding:32px 16px 18px 42px;box-shadow:inset 0 3px 8px rgba(0,0,0,.42)}.gbx-dm{position:absolute;top:11px;left:0;right:0;text-align:center;font-size:9.5px;font-weight:600;letter-spacing:.05em;color:#e3e0e6}.gbx-batt{position:absolute;left:13px;top:50%;transform:translateY(-50%);text-align:center;font-size:8px;font-weight:700;letter-spacing:.1em;color:#e3e0e6}.gbx-batt .led{display:block;width:9px;height:9px;border-radius:50%;background:#d8442e;margin:0 auto 5px;box-shadow:0 0 7px rgba(216,68,46,.85)}.gbx-lcd{background:#abb87a;border-radius:5px;padding:15px 16px;color:#2c3620;font-family:"VT323",ui-monospace,"SF Mono",Menlo,monospace;font-size:20px;line-height:1.3;box-shadow:inset 0 0 0 3px rgba(44,54,32,.22),inset 0 0 28px rgba(44,54,32,.15);background-image:repeating-linear-gradient(0deg,rgba(44,54,32,.05) 0 1px,transparent 1px 3px),repeating-linear-gradient(90deg,rgba(44,54,32,.03) 0 1px,transparent 1px 3px)}.gbx-lcd .r{display:flex;justify-content:space-between;align-items:center}.gbx-lcd .head{border-bottom:2px solid rgba(44,54,32,.42);padding-bottom:7px;margin-bottom:11px;font-size:14px}.gbx-lcd .col{font-size:13px;letter-spacing:.08em;color:rgba(44,54,32,.72)}.gbx-lcd .mm{font-size:28px;margin:2px 0 12px}.gbx-lcd .cur{animation:gbxblink 1.1s steps(1) infinite}@keyframes gbxblink{50%{opacity:0}}@media(prefers-reduced-motion:reduce){.gbx-lcd .cur{animation:none}}.gbx-lcd .intro{margin:0 0 12px;font-size:16px;line-height:1.35}.gbx-lcd .it{padding:9px 10px;display:flex;justify-content:space-between;font-size:20px}.gbx-lcd .it.sel{background:#2c3620;color:#abb87a;border-radius:3px}.gbx-lcd .cnt{font-size:13px;letter-spacing:.06em;color:rgba(44,54,32,.72);margin-top:12px}.gbx-lcd .foot{border-top:2px solid rgba(44,54,32,.42);margin-top:11px;padding-top:7px;font-size:13px}.gbx-word{margin:18px 6px 0;color:#2b3a6b;display:flex;align-items:baseline;gap:9px}.gbx-word b{font-weight:800;font-style:italic;font-size:22px}.gbx-word i{font-family:Georgia,serif;font-style:italic;font-size:28px;line-height:1}.gbx-word sup{font-family:"VT323",monospace;font-size:13px;color:#5f6a42}.gbx-ctl{display:flex;justify-content:space-between;align-items:center;margin:26px 8px 0}.gbx-dpad{position:relative;width:96px;height:96px;flex:none}.gbx-dpad span{position:absolute;background:#2a2a2c;box-shadow:inset 0 2px 3px rgba(255,255,255,.12),0 3px 5px rgba(0,0,0,.4)}.gbx-dpad .h{top:35px;left:0;width:96px;height:26px;border-radius:6px}.gbx-dpad .v{left:35px;top:0;width:26px;height:96px;border-radius:6px}.gbx-dpad .c{left:36px;top:36px;width:24px;height:24px;background:#202022;border-radius:50%}.gbx-ab{display:flex;gap:18px;align-items:flex-end;transform:rotate(-25deg)}.gbx-ab .k{position:relative}.gbx-ab .btn{width:46px;height:46px;border-radius:50%;background:radial-gradient(circle at 38% 30%,#cf4069,#a01f4e 72%);box-shadow:0 5px 7px rgba(0,0,0,.42),inset 0 2px 3px rgba(255,255,255,.35),inset 0 -4px 6px rgba(0,0,0,.32)}.gbx-ab .lbl{position:absolute;bottom:-21px;left:50%;transform:translateX(-50%) rotate(25deg);font-weight:800;font-size:15px;color:#2b3a6b}.gbx-se{display:flex;gap:40px;justify-content:center;margin:34px 0 0}.gbx-se .k{display:flex;flex-direction:column;align-items:center;gap:11px;transform:rotate(-25deg)}.gbx-se .cap{width:58px;height:15px;border-radius:9px;background:linear-gradient(#c6c4cb,#a9a7af);box-shadow:0 3px 4px rgba(0,0,0,.32),inset 0 1px 0 rgba(255,255,255,.6)}.gbx-se .lbl{font-weight:800;font-size:11px;letter-spacing:.05em;color:#2b3a6b}.gbx-phones{text-align:center;margin:34px 0 0;font-size:10px;font-weight:600;letter-spacing:.14em;color:#6d7666}.gbx-phones .jack{display:inline-block;width:9px;height:9px;border-radius:50%;border:1.5px solid #6d7666;margin-left:7px;vertical-align:middle}.gbx-grille{position:absolute;right:24px;bottom:22px;display:flex;gap:6px;transform:rotate(-25deg)}.gbx-grille i{width:4px;height:42px;border-radius:3px;background:linear-gradient(#c9c6bd,#b4b1a8)}@media(max-width:650px){.pocket-hero .gbx{transform:scale(.9)}}`;
  let html=`<!doctype html><html lang="${esc(languages[0]||'es')}" data-quality="${calidadSala}" data-mode="${mode}" data-layout="${layout}" data-profile="${profile}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow"><title>${name} · Presentación</title>${pocketFont}<style>
  :root{--primary:${primary};--accent:${accent};--bg:${background};--surface:${surface};--ink:${text};--radius:${radius}px;--shape:${shape};--mono:ui-monospace,"SF Mono",Menlo,monospace;--sans:${fontStack(config.theme?.fontStyle)}}*{box-sizing:border-box}html{scroll-snap-type:y mandatory;scroll-behavior:smooth}body{margin:0;background:var(--bg);color:var(--ink);font-family:var(--sans)}.slide{min-height:100vh;scroll-snap-align:start;display:grid;place-items:center;padding:${slidePad};position:relative;overflow:hidden;background:radial-gradient(80% 75% at 100% 0%,color-mix(in srgb,var(--accent) 24%,transparent),transparent 65%),linear-gradient(135deg,var(--primary),var(--bg))}.slide:nth-child(even){background:radial-gradient(80% 75% at 0% 100%,color-mix(in srgb,var(--accent) 20%,transparent),transparent 65%),linear-gradient(225deg,var(--primary),var(--bg))}.slide[data-has-image="true"]:before{content:"";position:absolute;inset:0;background-image:linear-gradient(105deg,color-mix(in srgb,var(--bg) 92%,transparent) 0%,color-mix(in srgb,var(--bg) 72%,transparent) 52%,color-mix(in srgb,var(--primary) 38%,transparent) 100%),var(--slide-image);background-size:cover;background-position:center;filter:saturate(.92);transform:scale(1.01)}.slide:after{content:"";position:absolute;width:36vw;height:36vw;border:6vw solid color-mix(in srgb,var(--accent) 12%,transparent);border-radius:var(--shape);right:-18vw;bottom:-22vw;transform:rotate(-12deg)}.inner{position:relative;z-index:1;width:min(1050px,100%)}html[data-layout="centered"] .inner{text-align:center;margin:auto}html[data-layout="centered"] h1,html[data-layout="centered"] h2,html[data-layout="centered"] p,html[data-layout="centered"] .close span{margin-left:auto;margin-right:auto}.num,.eyebrow{display:block;font:800 12px/1 var(--mono);letter-spacing:.18em;text-transform:uppercase;color:var(--accent);margin-bottom:28px}h1,h2{font-size:clamp(44px,8vw,100px);line-height:.98;letter-spacing:-.05em;margin:0;max-width:1000px}.cover h1{font-size:clamp(50px,9vw,118px)}.message{font-size:clamp(21px,3vw,38px);line-height:1.2;font-weight:700;max-width:920px;margin:34px 0 0}.detail{font-size:clamp(15px,1.6vw,20px);line-height:1.55;color:color-mix(in srgb,var(--ink) 66%,transparent);max-width:760px;margin:24px 0 0}.close strong{display:block;font-size:clamp(42px,7vw,90px);line-height:1;letter-spacing:-.045em}.close span{display:block;font-size:clamp(18px,2.5vw,30px);color:color-mix(in srgb,var(--ink) 68%,transparent);margin-top:28px;max-width:850px}.nav{position:fixed;z-index:9;right:18px;bottom:18px;background:color-mix(in srgb,var(--surface) 72%,transparent);backdrop-filter:blur(10px);border:1px solid color-mix(in srgb,var(--ink) 15%,transparent);border-radius:calc(var(--radius) + 16px);padding:10px 14px;font:700 10px/1 var(--mono);letter-spacing:.08em;color:color-mix(in srgb,var(--ink) 72%,transparent)}.languages{position:fixed;z-index:10;right:18px;top:18px;display:flex;background:color-mix(in srgb,var(--surface) 72%,transparent);border:1px solid color-mix(in srgb,var(--ink) 15%,transparent);border-radius:calc(var(--radius) + 16px);padding:4px;backdrop-filter:blur(10px)}.languages button{border:0;border-radius:calc(var(--radius) + 12px);padding:9px 11px;background:transparent;color:color-mix(in srgb,var(--ink) 68%,transparent);font:800 10px/1 var(--mono);cursor:pointer}.languages button[aria-pressed="true"]{background:var(--accent);color:var(--bg)}.slide-footer{position:absolute;left:18px;right:18px;bottom:14px;z-index:4;display:flex;justify-content:space-between;gap:12px;pointer-events:none;font:700 10px/1 var(--mono);letter-spacing:.08em;text-transform:uppercase;color:color-mix(in srgb,var(--ink) 55%,transparent)}.slide-footer-brand,.slide-footer-text{opacity:.9}.insert-slide{background:linear-gradient(145deg,color-mix(in srgb,var(--primary) 80%,#000),var(--bg))}.embed-slide .inner{width:min(1280px,100%)}.embed-frame{position:relative;margin-top:24px;aspect-ratio:16/9;border:1px solid color-mix(in srgb,var(--ink) 16%,transparent);border-radius:var(--radius);overflow:hidden;background:#05070a}.embed-frame iframe{display:block;width:100%;height:100%;border:0}.embed-frame[data-embed-idle] iframe{pointer-events:none}.embed-activate{position:absolute;inset:auto 12px 12px auto;z-index:2;border:1px solid color-mix(in srgb,var(--ink) 22%,transparent);border-radius:999px;padding:9px 13px;background:color-mix(in srgb,var(--surface) 88%,transparent);color:var(--ink);font:800 10px/1 var(--mono);letter-spacing:.08em;cursor:pointer;backdrop-filter:blur(10px)}.embed-frame:not([data-embed-idle]) .embed-activate{display:none}.embed-out{display:inline-block;margin-top:14px;color:color-mix(in srgb,var(--ink) 62%,transparent);font:700 11px/1.4 var(--mono);text-decoration:none;overflow-wrap:anywhere}.embed-out:hover{color:var(--accent)}.deck-slide{padding:0;background:#07090d}.deck-slide:after{display:none}.deck-slide img{display:block;width:100%;height:100vh;object-fit:contain;background:#07090d}.deck-copy{display:none}.deck-better img,.deck-best img{object-fit:cover;filter:saturate(.7) contrast(1.08) brightness(.62);transform:scale(1.015)}.deck-best img{filter:saturate(.9) contrast(1.12) brightness(.7)}.deck-better:before,.deck-best:before{content:"";position:absolute;z-index:1;inset:0;background:linear-gradient(90deg,rgba(4,8,15,.94) 0%,rgba(4,8,15,.72) 42%,rgba(4,8,15,.12) 76%),linear-gradient(0deg,rgba(4,8,15,.6),transparent 44%)}.deck-copy{position:absolute;z-index:2;left:7vw;bottom:15vh;width:min(760px,72vw);color:#fff}.deck-better .deck-copy,.deck-best .deck-copy{display:block}.deck-kicker{display:block;margin-bottom:25px;color:var(--accent);font:800 12px/1 var(--mono);letter-spacing:.2em;text-transform:uppercase}.deck-copy h2{font-size:clamp(46px,7.5vw,105px);line-height:.92;text-wrap:balance;text-shadow:0 3px 36px rgba(0,0,0,.55)}.deck-best .deck-kicker{color:#ffbe2e}.deck-best .deck-copy h2{font-family:Georgia,"Times New Roman",serif;font-weight:500;letter-spacing:-.04em}.deck-progress{position:absolute;left:18px;bottom:18px;z-index:3;padding:9px 12px;border-radius:999px;background:rgba(5,8,12,.76);color:#fff;font:800 9px/1 var(--mono);letter-spacing:.06em;backdrop-filter:blur(10px)}html[data-profile="minimal"] .slide:after{display:none}html[data-profile="editorial"] h1,html[data-profile="editorial"] h2{letter-spacing:-.035em}html[data-profile="friendly"] h1,html[data-profile="friendly"] h2{letter-spacing:-.03em}html[data-profile="immersive"] .slide:not([data-has-image="true"]):not(.deck-slide){background:radial-gradient(100% 100% at 85% 0%,color-mix(in srgb,var(--accent) 36%,transparent),transparent 65%),linear-gradient(145deg,var(--primary),var(--bg))}@media(max-width:650px){.slide:not(.deck-slide){padding:10vh 8vw}.nav{display:none}.deck-progress{left:10px;bottom:10px}.deck-copy{left:8vw;bottom:13vh;width:84vw}.deck-copy h2{font-size:clamp(38px,13vw,68px)}}
  </style><style>html[data-quality="good"]{--primary:#e9ece2;--accent:#789b3a;--bg:#f7f8f3;--surface:#fff;--ink:#18212a;--sans:Inter,-apple-system,"Segoe UI",Arial,sans-serif}html[data-quality="better"]{--primary:#12233e;--accent:#ffb000;--bg:#07101b;--surface:#111827;--ink:#f8fbff;--sans:Inter,-apple-system,"Segoe UI",Arial,sans-serif}html[data-quality="good"] .slide:not(.deck-slide){background:linear-gradient(135deg,#fff,#edf0e7)}html[data-quality="good"] .slide:not(.deck-slide):after{border-color:rgba(120,155,58,.13)}html[data-quality="better"] .slide:not(.deck-slide){background:radial-gradient(90% 80% at 85% 0%,rgba(255,176,0,.24),transparent 62%),linear-gradient(145deg,#12233e,#07101b)}html[data-quality="good"] .slide[data-has-image="true"]:before{display:none}html[data-quality="better"] .slide[data-has-image="true"]:before{display:block}html[data-quality="best"]{--primary:#0b1220;--accent:#ffbe2e;--bg:#05080f;--surface:#101826;--ink:#f7fafc;--sans:"Iowan Old Style",Georgia,"Times New Roman",serif}html[data-quality="best"] .slide:not(.deck-slide){background:radial-gradient(90% 80% at 12% 0%,rgba(255,190,46,.18),transparent 58%),linear-gradient(155deg,#101826,#05080f)}.best-figure{display:none;margin:0;padding:0;border:0}html[data-quality="best"] .slide:not(.deck-slide)[data-has-image="true"],html[data-quality="best"] .slide:not(.deck-slide)[data-has-video="true"]{display:grid;grid-template-columns:minmax(0,1.08fr) minmax(260px,.92fr);align-items:center;justify-items:stretch;gap:clamp(18px,3.5vw,48px);padding:8vh 5vw;place-items:unset}html[data-quality="best"] .slide:not(.deck-slide)[data-has-image="true"]:before,html[data-quality="best"] .slide:not(.deck-slide)[data-has-video="true"]:before{display:block;filter:saturate(.78) brightness(.42) contrast(1.05);transform:scale(1.04);background-image:linear-gradient(115deg,rgba(5,8,15,.88) 0%,rgba(5,8,15,.55) 46%,rgba(5,8,15,.22) 100%),var(--slide-image)}html[data-quality="best"] .slide:not(.deck-slide)[data-has-image="true"] .best-figure,html[data-quality="best"] .slide:not(.deck-slide)[data-has-video="true"] .best-figure{display:flex;align-items:center;justify-content:center;position:relative;z-index:2;width:100%;height:100%;order:2}html[data-quality="best"] .slide:not(.deck-slide)[data-has-image="true"] .inner,html[data-quality="best"] .slide:not(.deck-slide)[data-has-video="true"] .inner{order:1;z-index:3;width:min(640px,100%);max-width:none;padding:clamp(22px,3.2vw,42px);border:1px solid rgba(255,255,255,.14);border-radius:calc(var(--radius) + 18px);background:linear-gradient(160deg,rgba(16,24,38,.82),rgba(8,12,20,.68));box-shadow:0 24px 80px rgba(0,0,0,.45);backdrop-filter:blur(16px)}html[data-quality="best"] .best-figure img,html[data-quality="best"] .best-figure video{display:block;width:min(100%,540px);height:auto;max-height:min(72vh,640px);aspect-ratio:4/5;object-fit:cover;border-radius:calc(var(--radius) + 22px);border:1px solid rgba(255,255,255,.16);box-shadow:0 28px 70px rgba(0,0,0,.55),0 0 0 1px rgba(255,190,46,.12)}html[data-quality="best"] .best-figure img[hidden],html[data-quality="best"] .best-figure video[hidden]{display:none}html[data-quality="best"] .best-figure video{background:#05080f}html[data-quality="best"] .slide:not(.deck-slide)[data-has-image="true"] h1,html[data-quality="best"] .slide:not(.deck-slide)[data-has-image="true"] h2,html[data-quality="best"] .slide:not(.deck-slide)[data-has-image="true"] .close strong,html[data-quality="best"] .slide:not(.deck-slide)[data-has-video="true"] h1,html[data-quality="best"] .slide:not(.deck-slide)[data-has-video="true"] h2,html[data-quality="best"] .slide:not(.deck-slide)[data-has-video="true"] .close strong{font-size:clamp(34px,5.4vw,72px);line-height:1.02;letter-spacing:-.035em;text-wrap:balance;max-width:18ch}html[data-quality="best"] .slide:not(.deck-slide)[data-has-image="true"] .message,html[data-quality="best"] .slide:not(.deck-slide)[data-has-video="true"] .message{font-size:clamp(18px,2.1vw,28px);line-height:1.28;font-weight:650;max-width:34ch;margin-top:22px;color:rgba(247,250,252,.92)}html[data-quality="best"] .slide:not(.deck-slide)[data-has-image="true"] .detail,html[data-quality="best"] .slide:not(.deck-slide)[data-has-image="true"] .close span,html[data-quality="best"] .slide:not(.deck-slide)[data-has-image="true"] #coverSummary,html[data-quality="best"] .slide:not(.deck-slide)[data-has-video="true"] .detail,html[data-quality="best"] .slide:not(.deck-slide)[data-has-video="true"] .close span,html[data-quality="best"] .slide:not(.deck-slide)[data-has-video="true"] #coverSummary{font-size:clamp(14px,1.35vw,18px);line-height:1.55;max-width:42ch;margin-top:18px;color:rgba(247,250,252,.68)}html[data-quality="best"] .slide:not(.deck-slide)[data-has-image="true"] .num,html[data-quality="best"] .slide:not(.deck-slide)[data-has-image="true"] .eyebrow,html[data-quality="best"] .slide:not(.deck-slide)[data-has-video="true"] .num,html[data-quality="best"] .slide:not(.deck-slide)[data-has-video="true"] .eyebrow{margin-bottom:18px;letter-spacing:.16em}html[data-quality="best"] .slide:not(.deck-slide)[data-has-image="true"]:after,html[data-quality="best"] .slide:not(.deck-slide)[data-has-video="true"]:after{display:none}@media(max-width:900px){html[data-quality="best"] .slide:not(.deck-slide)[data-has-image="true"],html[data-quality="best"] .slide:not(.deck-slide)[data-has-video="true"]{grid-template-columns:1fr;padding:7vh 7vw 8vh}html[data-quality="best"] .slide:not(.deck-slide)[data-has-image="true"] .best-figure,html[data-quality="best"] .slide:not(.deck-slide)[data-has-video="true"] .best-figure{order:1;max-height:38vh}html[data-quality="best"] .slide:not(.deck-slide)[data-has-image="true"] .inner,html[data-quality="best"] .slide:not(.deck-slide)[data-has-video="true"] .inner{order:2}html[data-quality="best"] .best-figure img,html[data-quality="best"] .best-figure video{max-height:34vh;aspect-ratio:16/10;width:100%}}.deck-slide,.slide{transition:background .35s,color .35s}.deck-best .deck-copy h2{font-family:var(--sans);font-weight:750}.deck-best .deck-kicker{color:var(--accent)}html[data-quality="best"] .deck-best img{filter:saturate(.94) contrast(1.08) brightness(.66)}.section-nav{position:fixed;z-index:20;left:50%;top:18px;transform:translateX(-50%);display:flex;gap:3px;padding:4px;border:1px solid color-mix(in srgb,var(--ink) 15%,transparent);border-radius:999px;background:color-mix(in srgb,var(--surface) 78%,transparent);backdrop-filter:blur(12px)}.section-nav button{display:flex;align-items:center;gap:7px;border:0;border-radius:999px;padding:8px 11px;background:transparent;color:color-mix(in srgb,var(--ink) 64%,transparent);font:750 10px/1 var(--sans);cursor:pointer;white-space:nowrap}.section-nav button b{font:800 9px/1 var(--mono);color:var(--accent)}.section-nav button[aria-pressed="true"]{background:var(--accent);color:var(--bg)}.section-nav button[aria-pressed="true"] b{color:inherit}.section-nav button:disabled{display:none}@media(max-width:1050px){.section-nav button{padding:9px}.section-nav button span{display:none}}@media(max-width:650px){.section-nav{top:auto;bottom:14px}.section-nav button{padding:8px 10px}}</style></head><body><div class="languages" aria-label="Idiomas">${buttons}</div><nav class="section-nav" aria-label="Segmentos de la presentación">${sectionNav}</nav>${pocketHero}${beforeSlides}${earlyInserts}<section class="slide cover"${mediaFor('cover')?' data-slide-media':''} data-slide-key="cover" data-segment="proposal" data-section="proposal" data-image-index="0"${imageAttr(0)}${bestVideoAttr('cover')}>${mediaFor('cover')}<figure class="best-figure" aria-hidden="true"><img alt="" loading="lazy"><video class="best-figure-video" playsinline muted loop preload="metadata" hidden></video></figure><div class="inner"><span class="eyebrow" data-edit-field="hero.eyebrow">${esc(heroInicial?.eyebrow||`ADmiraNeXT × ${config.displayName}`)}</span><h1 id="coverTitle" data-edit-field="hero.title">${esc(heroInicial?.title)}</h1><p class="detail" id="coverSummary" data-edit-field="hero.summary">${esc(heroInicial?.summary)}</p></div>${foot('portada')}</section>${coverInsert}<section class="slide"${mediaFor('objective')?' data-slide-media':''} data-slide-key="objective" data-segment="proposal" data-section="proposal" data-image-index="1"${imageAttr(1)}${bestVideoAttr('objective')}>${mediaFor('objective')}<figure class="best-figure" aria-hidden="true"><img alt="" loading="lazy"><video class="best-figure-video" playsinline muted loop preload="metadata" hidden></video></figure><div class="inner"><span class="eyebrow" id="objectiveLabel" data-edit-field="labels.objective">${esc(etiquetasIniciales?.objective||locales.es.labels.objective)}</span><h2 id="objectiveText" data-edit-field="objective">${esc(objetivoInicial)}</h2></div>${foot('objetivo')}</section>${objectiveInsert}${slides}${embedSlides}<section class="slide"${mediaFor('closing')?' data-slide-media':''} data-slide-key="closing" data-segment="proposal" data-section="proposal" data-image-index="${baseBlocks.length+2}"${imageAttr(baseBlocks.length+2)}${bestVideoAttr('closing')}>${mediaFor('closing')}<figure class="best-figure" aria-hidden="true"><img alt="" loading="lazy"><video class="best-figure-video" playsinline muted loop preload="metadata" hidden></video></figure><div class="inner close"><span class="eyebrow" id="nextLabel" data-edit-field="labels.next">${esc(etiquetasIniciales?.next||locales.es.labels.next)}</span><strong id="closingTitle" data-edit-field="closing.title">${esc(cierreInicial?.title)}</strong><span id="closingAction" data-edit-field="closing.action">${esc(cierreInicial?.action)}</span></div></section>${afterSlides}<div class="nav" id="sequenceNav">↑ ↓ · F pantalla completa</div><script>
  ...
  const presentationState=window.__ADMIRA_PRESENTATION_STATE__={locales:${safeJson(locales)},languages:${safeJson(languages)},emptyLocale:${safeJson(visibleLocale({}))}${audienceMode?'':`,terminology:${safeJson(config.terminology||ideas.terminology||[])}`},sectionLabels:${safeJson(sectionLabels)},uiLabels:${safeJson(uiLabels)},displayName:${safeJson(config.displayName)},revision:${safeJson(ideas.updatedAt||'')},language:'es',quality:'${calidadSala}'};let slides=[...document.querySelectorAll('.slide')];let at=0;function refreshSlides(){slides=[...document.querySelectorAll('.slide')];at=Math.max(0,Math.min(slides.length-1,at));syncNav()}window.__ADMIRA_REFRESH_SLIDES__=refreshSlides;
  ...
  function syncBestFigures(){document.querySelectorAll('.slide[data-image-index]').forEach(slide=>{const figureRoot=slide.querySelector('.best-figure');if(!figureRoot)return;const img=figureRoot.querySelector('img');const video=figureRoot.querySelector('video.best-figure-video');const mediaVideo=slide.querySelector('[data-media-type="video"] [data-slide-media-element]');const videoUrl=(slide.getAttribute('data-best-video')||mediaVideo?.getAttribute('src')||'').trim();if(presentationState.quality==='best'&&videoUrl&&video){if(video.getAttribute('src')!==videoUrl)video.setAttribute('src',videoUrl);video.hidden=false;try{video.play?.().catch(()=>{})}catch(_){}if(img){img.removeAttribute('src');img.hidden=true}slide.dataset.hasVideo='true';figureRoot.style.display='';return}if(video){video.removeAttribute('src');video.hidden=true;try{video.pause?.()}catch(_){}}delete slide.dataset.hasVideo;const raw=(slide.style.getPropertyValue('--slide-image')||'').trim();let url='';if(raw.startsWith('url(')&&raw.endsWith(')')){url=raw.slice(4,-1).trim();if((url.startsWith('"')&&url.endsWith('"'))||(url.startsWith("'")&&url.endsWith("'")))url=url.slice(1,-1)}if(img){if(url&&presentationState.quality==='best'&&slide.dataset.hasImage==='true'){if(img.getAttribute('src')!==url)img.setAttribute('src',url);img.hidden=false}else{img.removeAttribute('src');img.hidden=true}}})}
  function applyImageSet(set){const imageSlides=Array.isArray(set?.slides)?set.slides:[],allowedImageUrl=new RegExp('^/presentaciones/${client}/images/[a-z0-9._-]+$','i'),adapted=[];let imported=0;document.querySelectorAll('[data-image-index]').forEach(slide=>{const image=imageSlides[Number(slide.dataset.imageIndex)],url=String(image?.url||'');if(image?.status!=='ready'||image?.textFreeVerified!==true||!allowedImageUrl.test(url))return;slide.dataset.hasImage='true';slide.style.setProperty('--slide-image',"url('"+url+"')");adapted.push(url);imported+=1});if(adapted.length)document.querySelectorAll('[data-deck-image]').forEach((image,index)=>{const url=adapted[index%adapted.length];for(const language of ['es','ca','en'])image.setAttribute('data-src-best-'+language,url)});applyDeckSource();syncBestFigures();return imported}
  async function syncImages(){try{const response=await fetch('api/images',{headers:{accept:'application/json'},cache:'no-store'});if(!response.ok)return;const data=await response.json();applyImageSet(data.imageSet);const laminas=data.imageSet?.slides||[],cerrado=laminas.length>0&&laminas.every(lamina=>lamina.status==='ready'||lamina.status==='failed');if(data.imageSet?.status==='complete'||cerrado)clearInterval(imageTimer)}catch(_){}}
  ...
  // PDF al vuelo: el botón «PDF» y la tecla P abren el diálogo de impresión con los estilos de
  // impresión de arriba (una lámina por página). El cliente se lleva el deck sin esperar a nadie.
  document.querySelector('[data-print]')?.addEventListener('click',()=>{try{window.print()}catch(_){}});
  // Documento de trabajo: el portal enlaza el deck con ?pdf=1; se destaca el botón PDF y se dice cómo.
  if(new URLSearchParams(location.search).get('pdf')==='1'){var bp=document.querySelector('[data-print]');if(bp){bp.classList.add('deck-print-hint');bp.textContent='PDF ↓';bp.title=(bp.title||'')+' · Documento de trabajo: pulsa aquí o la tecla P';try{bp.focus()}catch(_){}}}
  // TÁCTIL (FLT-100018 b): el desplazamiento vertical ya lo hace el scroll-snap; un gesto
  // HORIZONTAL claro (≥60 px y más horizontal que vertical) pasa de lámina, como en un móvil.
  (function(){var x0=null,y0=null;addEventListener('touchstart',function(e){var t=e.changedTouches&&e.changedTouches[0];if(!t)return;x0=t.clientX;y0=t.clientY},{passive:true});addEventListener('touchend',function(e){var t=e.changedTouches&&e.changedTouches[0];if(x0==null||!t)return;var dx=t.clientX-x0,dy=t.clientY-y0;x0=y0=null;if(Math.abs(dx)<60||Math.abs(dx)<Math.abs(dy)*1.2)return;if(e.target&&e.target.closest&&e.target.closest('.embed-frame,input,textarea,[contenteditable]'))return;go(at+(dx<0?1:-1))},{passive:true})})();
  window.__ADMIRA_APPLY_LANGUAGE__=applyLanguage;window.__ADMIRA_APPLY_QUALITY__=applyQuality;window.__ADMIRA_SYNC_NAV__=syncNav;addEventListener('keydown',event=>{if(event.target?.isContentEditable||event.target?.closest?.('input,textarea,select,button'))return;if(['ArrowDown','ArrowRight',' '].includes(event.key)){event.preventDefault();go(at+1)}if(['ArrowUp','ArrowLeft'].includes(event.key)){event.preventDefault();go(at-1)}if(event.key.toLowerCase()==='f')enterFullscreen();if(event.key.toLowerCase()==='p'&&!event.metaKey&&!event.ctrlKey){event.preventDefault();try{window.print()}catch(_){}}});addEventListener('scroll',()=>{let best=0,dist=Infinity;slides.forEach((slide,index)=>{const delta=Math.abs(slide.getBoundingClientRect().top);if(delta<dist){dist=delta;best=index}});at=best;syncNav()},{passive:true});// PANTALLA COMPLETA HASTA QUE SE PULSE ESC (Carlos, 02-09-2026). El cliente entra por el
  ...
  const query=new URLSearchParams(location.search),requestedLanguage=query.get('lang'),requestedQuality=query.get('quality');applyLanguage(presentationState.locales[requestedLanguage]?requestedLanguage:'${esc(languages[0]||'es')}');applyQuality(['good','better','best'].includes(requestedQuality)?requestedQuality:presentationState.quality);syncNav();const wantsImages=${(Array.isArray(config.outputs)&&config.outputs.includes('backgrounds'))||['better','best'].includes(initialQuality)?'true':'false'};const imageTimer=${!setCerrado&&((Array.isArray(config.outputs)&&config.outputs.includes('backgrounds')&&imageSet?.status!=='complete')||['better','best'].includes(initialQuality))?'setInterval(syncImages,10000)':'0'};if(imageTimer){syncImages();addEventListener('pagehide',()=>clearInterval(imageTimer),{once:true})}if(wantsImages&&${generaLaminas?'true':'false'}&&(presentationState.quality==='better'||presentationState.quality==='best'))ensureBetterImages()
  </script></body></html>`;
  ...
  // PDF NATIVO POR IMPRESIÓN (MorfeoMacMini, 6-sep-2026 · FLT-100018 b): «presentarlo bien y
  // rápido» incluye dejarle al cliente un PDF sin esperar al productor. Cada lámina es una
  // página apaisada; las barras, los botones y las webs vivas (iframes) no se imprimen —de una
  // web embebida queda su título y su enlace—; los colores del tema se conservan.
  const printCss=`.deck-print-hint{outline:2px solid var(--accent);box-shadow:0 0 0 4px color-mix(in srgb,var(--accent) 35%,transparent)}@media print{@page{size:landscape;margin:10mm}html,body{scroll-snap-type:none!important;overflow:visible!important;height:auto!important;background:var(--bg)!important;-webkit-print-color-adjust:exact;print-color-adjust:exact}.top,nav,.deck-print,[data-client-fullscreen],.embed-activate,.embed-frame,.slide-media-control,[data-presenter-only]{display:none!important}.slide{min-height:0!important;height:auto!important;display:block!important;page-break-after:always;break-after:page;page-break-inside:avoid;break-inside:avoid;padding:14mm 16mm!important;scroll-snap-align:none!important}.slide:last-child{page-break-after:auto;break-after:auto}.embed-slide .embed-out{display:inline-block!important;font-size:14pt}.inner{max-width:none!important}}`;
  html=html.replace('</head>',`<style>${responsiveGoodCss}${responsiveDetailCss}${brandIdentityCss}${printCss}${pocketCss}</style></head>`);
