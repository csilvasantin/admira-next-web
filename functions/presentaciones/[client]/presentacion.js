...
import {publicBudget,formatMoney} from '../_budget.js';
...
function visibleLocale(value={},fallback={}){
  return {
    ...
    budget:publicBudget(value.budget||fallback.budget)
  };
}
...
const sectionLabels={
  es:{who:'Quiénes somos',what:'Qué hacemos',how:'Cómo lo hacemos',proposal:'Qué proponemos',budget:'Presupuesto'},
  ca:{who:'Qui som',what:'Què fem',how:'Com ho fem',proposal:'Què proposem',budget:'Pressupost'},
  en:{who:'Who we are',what:'What we do',how:'How we do it',proposal:'What we propose',budget:'Quote'}
};
const budgetLabels={
  title:{es:'Presupuesto',ca:'Pressupost',en:'Quote'},
  concept:{es:'Concepto',ca:'Concepte',en:'Item'},
  price:{es:'Precio',ca:'Preu',en:'Price'},
  discount:{es:'Dto. %',ca:'Dto. %',en:'Disc. %'},
  iva:{es:'IVA %',ca:'IVA %',en:'VAT %'},
  amount:{es:'Importe',ca:'Import',en:'Amount'},
  base:{es:'Base',ca:'Base',en:'Base'},
  discounts:{es:'Descuentos',ca:'Descomptes',en:'Discounts'},
  tax:{es:'IVA',ca:'IVA',en:'VAT'},
  total:{es:'Total',ca:'Total',en:'Total'},
  empty:{es:'Sin partidas',ca:'Sense partides',en:'No line items'}
};
function localeAttr(map){
  return `data-title-es="${esc(map.es)}" data-title-ca="${esc(map.ca)}" data-title-en="${esc(map.en)}"`;
}
function budgetSlideMarkup(ideas){
  const budget=publicBudget(ideas?.budget);
  const rows=budget.lines.length
    ? budget.lines.map(line=>`<tr data-budget-line="${esc(line.id)}"><td data-budget-concept>${esc(line.concept)}</td><td data-budget-price>${esc(formatMoney(line.price))}</td><td data-budget-discount>${esc(String(line.discount))}</td><td data-budget-iva>${esc(String(line.iva))}</td><td data-budget-amount>${esc(formatMoney(line.amount))}</td></tr>`).join('')
    : `<tr class="budget-empty-row"><td colspan="5" data-budget-empty ${localeAttr(budgetLabels.empty)}>${esc(budgetLabels.empty.es)}</td></tr>`;
  return `<section class="slide budget-slide" data-slide-key="budget" data-section="budget" data-segment="budget"><div class="inner"><span class="eyebrow" data-budget-kicker ${localeAttr(budgetLabels.title)}>${esc(budgetLabels.title.es)}</span><h2 data-budget-heading ${localeAttr(budgetLabels.title)}>${esc(budgetLabels.title.es)}</h2><table class="budget-table"><thead><tr><th ${localeAttr(budgetLabels.concept)}>${esc(budgetLabels.concept.es)}</th><th ${localeAttr(budgetLabels.price)}>${esc(budgetLabels.price.es)}</th><th ${localeAttr(budgetLabels.discount)}>${esc(budgetLabels.discount.es)}</th><th ${localeAttr(budgetLabels.iva)}>${esc(budgetLabels.iva.es)}</th><th ${localeAttr(budgetLabels.amount)}>${esc(budgetLabels.amount.es)}</th></tr></thead><tbody data-budget-body>${rows}</tbody><tfoot><tr><th ${localeAttr(budgetLabels.base)}>${esc(budgetLabels.base.es)}</th><td colspan="4" data-budget-total="base">${esc(formatMoney(budget.totals.base))}</td></tr><tr><th ${localeAttr(budgetLabels.discounts)}>${esc(budgetLabels.discounts.es)}</th><td colspan="4" data-budget-total="discountTotal">${esc(formatMoney(budget.totals.discountTotal))}</td></tr><tr><th ${localeAttr(budgetLabels.tax)}>${esc(budgetLabels.tax.es)}</th><td colspan="4" data-budget-total="ivaTotal">${esc(formatMoney(budget.totals.ivaTotal))}</td></tr><tr class="budget-grand"><th ${localeAttr(budgetLabels.total)}>${esc(budgetLabels.total.es)}</th><td colspan="4" data-budget-total="total">${esc(formatMoney(budget.totals.total))}</td></tr></tfoot></table></div></section>`;
}
const uiLabels={
...
export async function onRequestGet(context){
  ...
  const query=new URLSearchParams(location.search),requestedLanguage=query.get('lang'),requestedQuality=query.get('quality');applyLanguage(presentationState.locales[requestedLanguage]?requestedLanguage:'${esc(languages[0]||'es')}');applyQuality(['good','better','best'].includes(requestedQuality)?requestedQuality:presentationState.quality);syncNav();const wantsImages=${(Array.isArray(config.outputs)&&config.outputs.includes('backgrounds'))||['better','best'].includes(initialQuality)?'true':'false'};const imageTimer=${(Array.isArray(config.outputs)&&config.outputs.includes('backgrounds')&&imageSet?.status!=='complete')||['better','best'].includes(initialQuality)?'setInterval(syncImages,10000)':'0'};if(imageTimer){syncImages();addEventListener('pagehide',()=>clearInterval(imageTimer),{once:true})}if(wantsImages&&(presentationState.quality==='better'||presentationState.quality==='best'))ensureBetterImages()
  </script></body></html>`;
  ...
  const printCss=`.deck-print-hint{outline:2px solid var(--accent);box-shadow:0 0 0 4px color-mix(in srgb,var(--accent) 35%,transparent)}@media print{@page{size:landscape;margin:10mm}html,body{scroll-snap-type:none!important;overflow:visible!important;height:auto!important;background:var(--bg)!important;-webkit-print-color-adjust:exact;print-color-adjust:exact}.top,nav,.deck-print,[data-client-fullscreen],.embed-activate,.embed-frame,.slide-media-control,[data-presenter-only],.budget-edit-tools{display:none!important}.slide{min-height:0!important;height:auto!important;display:block!important;page-break-after:always;break-after:page;page-break-inside:avoid;break-inside:avoid;padding:14mm 16mm!important;scroll-snap-align:none!important}.slide:last-child{page-break-after:auto;break-after:auto}.embed-slide .embed-out{display:inline-block!important;font-size:14pt}.inner{max-width:none!important}.budget-slide{page-break-inside:avoid;break-inside:avoid}.budget-table{font-size:11pt;width:100%}}`;
  const budgetCss=`.budget-slide .inner{width:min(1100px,100%)}.budget-table{width:100%;border-collapse:collapse;margin-top:28px;font-size:clamp(14px,1.5vw,18px);background:color-mix(in srgb,var(--surface) 55%,transparent)}.budget-table th,.budget-table td{padding:10px 12px;border-bottom:1px solid color-mix(in srgb,var(--ink) 14%,transparent);text-align:right;vertical-align:middle}.budget-table th:first-child,.budget-table td:first-child{text-align:left}.budget-table thead th{font:800 11px/1 var(--mono);letter-spacing:.08em;text-transform:uppercase;color:var(--accent)}.budget-table tfoot th{text-align:left;font-weight:700}.budget-table tfoot .budget-grand td,.budget-table tfoot .budget-grand th{font-size:1.15em;font-weight:800;border-top:2px solid var(--accent)}.budget-empty-row td{text-align:center!important;color:color-mix(in srgb,var(--ink) 55%,transparent);font-style:italic}.budget-table input{width:100%;min-width:0;border:1px solid color-mix(in srgb,var(--ink) 18%,transparent);border-radius:8px;padding:7px 8px;background:color-mix(in srgb,var(--surface) 88%,transparent);color:var(--ink);font:inherit}.budget-row-remove,.budget-add-row{border:1px solid color-mix(in srgb,var(--ink) 18%,transparent);border-radius:8px;padding:7px 10px;background:transparent;color:var(--ink);font:800 9px/1 var(--mono);cursor:pointer}.budget-add-row{margin-top:14px}html[data-quality="good"] .budget-table{background:rgba(255,255,255,.62)}html[data-quality="better"] .budget-table,html[data-quality="best"] .budget-table{background:rgba(16,24,38,.48)}@media print{.budget-slide{page-break-inside:avoid}.budget-table{font-size:11pt}}`;
  html=html.replace('</head>',`<style>${responsiveGoodCss}${responsiveDetailCss}${brandIdentityCss}${printCss}${budgetCss}</style></head>`);
