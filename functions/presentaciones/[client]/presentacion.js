...
export async function onRequestGet(context){
  ...
  function syncNav(){const slide=slides[at],section=slide?.dataset.section||'proposal',label=presentationState.sectionLabels[presentationState.language]?.[section]||presentationState.sectionLabels.es[section]||'Presentación',nav=document.getElementById('sequenceNav');nav.textContent=label+' · '+(at+1)+'/'+slides.length+' · ↑ ↓ · F'+(nav.dataset.editorHint||'');document.querySelectorAll('[data-section-target]').forEach(button=>button.setAttribute('aria-pressed',button.dataset.sectionTarget===section?'true':'false'))}
