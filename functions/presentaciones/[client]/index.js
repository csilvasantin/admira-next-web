...
function statusCopy(item){
  ...
  if(status==='complete') return `Finalizado${dateTime(item.completedAt||item.updatedAt)?` · ${dateTime(item.completedAt||item.updatedAt)}`:''}`;
  if(status==='error'){
  ...
  if(status==='queued'){
    const since=dateTime(item?.requestedAt||item?.updatedAt);
...
export async function onRequestGet(context){
  ...
  const [config,generation,imageSet]=await Promise.all([
  ...
  const allOutputs=['website','audio','video','pdf','powerpoint','documents','infographic','credits','postcredits'];
