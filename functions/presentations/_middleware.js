 ...
 *
 * CLIENT ISOLATION: every space has its own Cloudflare secret and signed cookie.
 * Lenovo credentials cannot open La Caixa or Clear Channel. Every resource below
 * /presentations/<client>/ inherits the first path segment, including media and docs.
 *
...
export async function onRequest(context){
  ...
  // First segment = client. Nested resources keep the same isolation boundary.
  const rel = url.pathname.replace(/^\/presentations\/?/i, '').replace(/^\/+|\/+$/g, '');
